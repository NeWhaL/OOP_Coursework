...
GameObject::GameObject(sf::Vector2f coordinates, float speed, float health,
    ...
    : coordinates(coordinates), speed(speed), health(health),
      main_sprite(new sf::Sprite), amount_sprite(amount_sprite) {
  ...
  radius_hitbox_head = bounds_sprite.height / 2;
}
...
sf::Vector2f GameObject::GetPositionHead() const {
  return main_sprite->getPosition();
}
...
TypeObject GameObject::GetTypeObject() const { return type; }
